...
const STATUS_LABEL: Record<PersonalBill["status"], string> = {
  paid: "Al día",
  overdue: "Vencido",
...
const STATUS_STYLE: Record<PersonalBill["status"], string> = {
...
export function PersonalBillsView() {
  ...
  async function loadBills() {
    const res = await fetch("/api/personal-bills");
  ...
  useEffect(() => {
    // Carga inicial al montar. Sin Realtime: dato de un solo usuario, nadie
    // más lo edita.
  ...
  return (
    <main className="mx-auto flex max-w-md flex-col gap-5 p-5">
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-ink-soft">No hay pagos recurrentes personales registrados.</p>
      ) : (
        <ul className="flex flex-col gap-3">
          {bills.map((bill) => (
            <PersonalBillRow key={bill.id} bill={bill} onChanged={loadBills} />
          ))}
        </ul>
      )}
    </main>
...
function PersonalBillRow({
  ...
  async function handleEdit(event: FormEvent) {
    ...
    try {
      const res = await fetch(`/api/personal-bills/${bill.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name,
  ...
  async function handlePay(event: FormEvent) {
    ...
    try {
      const res = await fetch(`/api/personal-bills/${bill.id}/pay`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete() {
    ...
    const res = await fetch(`/api/personal-bills/${bill.id}`, { method: "DELETE" });
    if (!res.ok) {
      const data = await res.json();
