...
export function PersonalExpensesView({ monthlyIncome }: { monthlyIncome: number | null }) {
  ...
  return (
    <main className="mx-auto flex max-w-md flex-col gap-5 p-5">
      ...
      <PersonalBudgets refreshKey={expenses.length} />
...
function PersonalBudgets({ refreshKey }: { refreshKey: number }) {
  ...
  useEffect(() => {
    ...
    // reflejar cuánto se lleva gastado del presupuesto). Sin Realtime, mismo
  ...
  async function handleAdd(event: FormEvent) {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`/api/personal-budgets/${newCategory}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleRemove(category: string) {
    ...
    await fetch(`/api/personal-budgets/${category}`, { method: "DELETE" });
    await loadBudgets();
  }
    ...
    <div className="rounded-2xl border border-rule bg-surface p-4">
      ...
      {budgets.length === 0 && !isAdding && (
        <p className="mt-2 text-xs text-ink-soft">
          Aún no tienes presupuestos definidos por categoría.
        </p>
      )}
